...
def extract_code(string):
    ...
    if matches:
        ...
        filtered_matches = [
            ...
            if "@Test" in match and "import" in match  # 不要求 class
        ]
        if filtered_matches:
...
def generate_test_case_cot(context_d_1, context_d_3, package, imports, method_id, project_name, class_name, test_num,
                           save_dir):
    """
    ...
    if context_type == 1:
        ...
        if remain_prompt_tokens(messages_intention) < 0:
            function_intention_context["information"] = _remove_imports_context(
                function_intention_context["information"], imports, package)
            messages_intention = generate_messages(TEMPLATE_NO_DEPS_INTENTION, function_intention_context)
        ...
        if remain_prompt_tokens(messages_intention) < 0:
            function_intention_context["full_fm"] = _remove_imports_context(function_intention_context["full_fm"],
                                                                            imports, package)
            messages_intention = generate_messages(TEMPLATE_WITH_DEPS_INTENTION, function_intention_context)
    ...
    try:
        ...
        while rounds < max_rounds:
            ...
            # 第一轮为该焦点方法生成测试用例   现在需要修改流程  需要先产生函数意图和分支信息再产生测试用例其他不变
                ...
                # 使用基于思维链的方法生成测试用例
                messages = generate_test_case_cot(context_d_1, context_d_3, package, imports, method_id,
                                                       project_name, class_name, test_num, save_dir)
...
def start_whole_process(source_dir, result_path, multiprocess=False, repair=True):
    ...
    if multiprocess:
        ...
        # -------- 从 futures 中读取 rounds 累加 --------
        for f in concurrent.futures.as_completed(futures):
            try:
                rounds_used = f.result()
                # 只在返回值为 int 时累加，None 或其它类型都当作 0 处理
                if isinstance(rounds_used, int):
                    total_rounds += rounds_used
                elif rounds_used is not None:
                    # 这里可以根据需要写个 warning
                    print("[Warn] whole_process returned non-int value:", rounds_used)
            except Exception as e:
        ...
        for idx, file_path in enumerate(file_paths):
            ...
            for test_num in range(1, test_number + 1):
                ...
                rounds_used = whole_process(
                    test_num, base_name, base_dir, repair, submits, total
                )
