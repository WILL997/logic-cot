...
#dataset_path 定义了数据集所在的目录路径。
#db 创建了一个数据库连接实例，用于数据库的后续操作。
dataset_path = "../dataset/"
db = database()

#生成文件名，通常用于存储与方法相关的数据。根据传入的参数，它会生成一个具有特定格式的文件名。
...
# create_dataset_dirs 函数的作用是创建一组目录，用于存储数据集的不同类型的数据文件
...
#函数的作用是生成指定类的最小签名信息，包括类的签名、字段信息和构造函数信息。
...
#该函数通过查询数据库，生成并返回一组方法签名，包含类的签名、字段信息、getter 和 setter 方法、构造函数以及其他指定方法的签名
...
#函数的作用是生成一个类的完整签名信息。这包括类的签名、字段信息、所有方法的签名等。
...
#生成一个 "焦点方法"（focal method）的完整上下文。这个上下文包括了类的签名、导入的内容、构造函数、字段信息、相关依赖方法的签名等。
...
#检查指定的类是否存在于某个项目中。它通过查询数据库来判断一个类是否在指定的项目中。
