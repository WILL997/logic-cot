...
class database(object):
    ...
    #用于从数据库中执行 SELECT 查询。该方法接受几个参数并生成相应的 SQL 查询语句，执行查询并返回结果。
    def select(self, table_name: str = "", conditions=None, result_cols: (str, list) = "*", script=None):
        ...
        return result
    #定义了一个 insert 方法，用于向指定的数据库表中插入数据
...
#用于在数据库中创建两个表：class 和 method。这两个表用于存储与项目相关的类和方法信息
def create_table():
    """
