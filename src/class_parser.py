...
class ClassParser():
    ...
    # 生成数组类型
    ARRAY_TYPES = set([ptype + '[]' for ptype in PRIMITIVE_TYPES])
    
    # 所有允许的类型
    ALLOWED_TYPES = PRIMITIVE_TYPES
    #ALLOWED_TYPES = PRIMITIVE_TYPES.union(ARRAY_TYPES)
            ...
            for child in (child for child in _class.children if child.type == 'class_body'):
                for _, node in enumerate(child.children):
                    if node.type == 'method_declaration' or node.type == 'constructor_declaration':
                        ...
                        elif ClassParser.is_method_with_allowed_param_types(node, content):
                            # Read Method metadata
                            method_metadata = ClassParser.get_function_metadata(class_identifier, node,
                                                                                class_metadata['fields'], content)
                            methods.append(method_metadata)
    ...
    @staticmethod
    def is_method_with_allowed_param_types(method_node, blob: str) -> bool:
        """
        检查方法的参数类型是否都是允许的类型(基本类型或包装类)
        """
        # 查找方法的参数节点
        for child in method_node.children:
            if child.type == 'formal_parameters':
                for param_child in child.named_children:
                    # 获取参数类型
                    if len(param_child.named_children) >= 1:
                        type_index = 0
                        # 如果有final修饰符，类型位置向后移动
                        if 'final' in ClassParser.match_from_span(param_child.named_children[0], blob):
                            type_index = 1
                        
                        if type_index < len(param_child.named_children):
                            type_node = param_child.named_children[type_index]
                            type_name = ClassParser.match_from_span(type_node, blob)
                            # 如果类型不是允许的类型，返回False
                            if type_name not in ClassParser.ALLOWED_TYPES:
                                return False
        return True
